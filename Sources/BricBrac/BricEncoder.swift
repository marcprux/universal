...
open class BricolageEncoder<B: Bricolage> { // TODO: : TopLevelEncoder
    ...
    public struct OutputFormatting : OptionSet {
        ...
        public static var prettyPrinted: OutputFormatting { return OutputFormatting(rawValue: 1 << 0) }
        
        /// Produce JSON with dictionary keys sorted in lexicographic order.
        @available(macOS 10.13, iOS 11.0, watchOS 4.0, tvOS 11.0, *)
        public static var sortedKeys: OutputFormatting { return OutputFormatting(rawValue: 1 << 1) }
    }
    ...
    /// Encodes the given top-level value and returns its Bric representation.
    ///
    /// - parameter value: The value to encode.
    /// - returns: A new `Bric` value containing the encoded object.
    /// - throws: `EncodingError.invalidValue` if a non-conforming floating-point value is encountered during encoding, and the encoding strategy is `.throw`.
    /// - throws: An error if any value throws an error during encoding.
    open func encode<T : Encodable>(_ value: T) throws -> B {
        return try B.parse(String(data: encodeData(value), encoding: .utf8) ?? "")
    }

    /// Encodes the given top-level value and returns its NSObject representation.
    ///
    /// - parameter value: The value to encode.
    /// - returns: A new `Bric` value containing the encoded object.
    /// - throws: `EncodingError.invalidValue` if a non-conforming floating-point value is encountered during encoding, and the encoding strategy is `.throw`.
    /// - throws: An error if any value throws an error during encoding.
    open func encodeObject<T : Encodable>(_ value: T) throws -> NSObject {
        let encoder = _BricolageEncoder(options: self.options)
        guard let topLevel = try encoder.wrap(value) else {
    ...
    /// - throws: `EncodingError.invalidValue` if a non-conforming floating-point value is encountered during encoding, and the encoding strategy is `.throw`.
    /// - throws: An error if any value throws an error during encoding.
    open func encodeData<T : Encodable>(_ value: T) throws -> Data {
        let topLevel = try encodeObject(value)
...
// MARK: - _BricolageEncoder
fileprivate class _BricolageEncoder<B: Bricolage> : Encoder {
    // MARK: Properties
    /// The encoder's storage.
    fileprivate var storage: _BricEncodingStorage
    
    /// Options set on the top-level encoder.
    fileprivate let options: BricolageEncoder<B>._Options
    ...
    fileprivate init(options: BricolageEncoder<B>._Options, codingPath: [CodingKey] = []) {
        self.options = options
        self.storage = _BricEncodingStorage()
    ...
    public func container<Key>(keyedBy: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        let container = _BricKeyedEncodingContainer<B, Key>(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        return _BricUnkeyedEncodingContainer(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    }
...
fileprivate struct _BricEncodingStorage {
    // MARK: Properties
    /// The container stack.
...
fileprivate struct _BricKeyedEncodingContainer<B : Bricolage, K : CodingKey> : KeyedEncodingContainerProtocol {
    typealias Key = K
    
    // MARK: Properties
    /// A reference to the encoder we're writing to.
    private let encoder: _BricolageEncoder<B>
    ...
    fileprivate init(referencing encoder: _BricolageEncoder<B>, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = BricEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
            return _BricKey(stringValue: newKeyString, intValue: key.intValue)
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> {
        ...
        let container = _BricKeyedEncodingContainer<B, NestedKey>(referencing: self.encoder, codingPath: self.codingPath, wrapping: dictionary)
        return KeyedEncodingContainer(container)
    }
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        ...
        self.codingPath.append(key)
        defer { self.codingPath.removeLast() }
        return _BricUnkeyedEncodingContainer(referencing: self.encoder, codingPath: self.codingPath, wrapping: array)
    }
    
    public mutating func superEncoder() -> Encoder {
        return _BricReferencingEncoder<B>(referencing: self.encoder, key: _BricKey.super, convertedKey: _converted(_BricKey.super), wrapping: self.container)
    }
    
    public mutating func superEncoder(forKey key: Key) -> Encoder {
        return _BricReferencingEncoder<B>(referencing: self.encoder, key: key, convertedKey: _converted(key), wrapping: self.container)
    }
}

fileprivate struct _BricUnkeyedEncodingContainer<B : Bricolage> : UnkeyedEncodingContainer {
    // MARK: Properties
    /// A reference to the encoder we're writing to.
    private let encoder: _BricolageEncoder<B>
    ...
    // MARK: - Initialization
    /// Initializes `self` with the given references.
    fileprivate init(referencing encoder: _BricolageEncoder<B>, codingPath: [CodingKey], wrapping container: NSMutableArray) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
    ...
    public mutating func encode(_ value: Float)  throws {
        // Since the float may be invalid and throw, the coding path needs to contain this key.
        self.encoder.codingPath.append(_BricKey(index: self.count))
        defer { self.encoder.codingPath.removeLast() }
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type) -> KeyedEncodingContainer<NestedKey> {
        self.codingPath.append(_BricKey(index: self.count))
        defer { self.codingPath.removeLast() }
    ...
    public mutating func superEncoder() -> Encoder {
        return _BricReferencingEncoder<B>(referencing: self.encoder, at: self.container.count, wrapping: self.container)
    }
}

extension _BricolageEncoder : SingleValueEncodingContainer {
...
extension _BricolageEncoder {
    ...
    fileprivate func box(_ date: Date) throws -> NSObject {
        ...
        case .iso8601:
            if #available(macOS 10.12, iOS 10.0, watchOS 3.0, tvOS 11.0, *) {
    ...
    fileprivate func box<T : Encodable>(_ value: T) throws -> NSObject {
        return try self.wrap(value) ?? NSDictionary()
    }
    
    fileprivate func wrap<T : Encodable>(_ value: T) throws -> NSObject? {
        ...
        // The value should request a container from the _BricolageEncoder.
        let depth = self.storage.count
        do {
...
// MARK: - _BricReferencingEncoder
/// _BricReferencingEncoder is a special subclass of _BricolageEncoder which has its own storage, but references the contents of a different encoder.
/// It's used in superEncoder(), which returns a new encoder for encoding a superclass -- the lifetime of the encoder should not escape the scope it's created in, but it doesn't necessarily know when it's done being used (to write to the original container).
fileprivate class _BricReferencingEncoder<B : Bricolage> : _BricolageEncoder<B> {
    ...
    fileprivate let encoder: _BricolageEncoder<B>
    ...
    fileprivate init(referencing encoder: _BricolageEncoder<B>, at index: Int, wrapping array: NSMutableArray) {
        self.encoder = encoder
        self.reference = .array(array, index)
        super.init(options: encoder.options, codingPath: encoder.codingPath)
        
        self.codingPath.append(_BricKey(index: index))
    }
    
    /// Initializes `self` by referencing the given dictionary container in the given encoder.
    fileprivate init(referencing encoder: _BricolageEncoder<B>,
...
fileprivate extension EncodingError {
    ...
    static func _invalidFloatingPointValue<T : FloatingPoint>(_ value: T, at codingPath: [CodingKey]) -> EncodingError {
        ...
        let debugDescription = "Unable to encode \(valueDescription) directly in JSON. Use BricEncoder.NonConformingFloatEncodingStrategy.convertToString to specify how the value should be encoded."
